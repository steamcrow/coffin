(function() {
  ...
  window.CCFB.refreshUI = () => {
    ...
    if (document.getElementById("f-description"))
      document.getElementById("f-description").textContent = faction?.description || "";
    ...
    const renderStats = (u) => {
      ...
      return config.map(s =>
        u[s.k] != null
          ? `<span class="cc-stat-badge"><span class="cc-stat-label ${s.c}">${s.l}</span><span class="cc-stat-value">${u[s.k]}</span></span>`
          : ''
      ).join('');
    };
    ...
    if (lib) {
      ...
      (faction?.units || []).forEach(u => {
        ...
        const abTags = (u.abilities || []).map(a =>
          `<span style="font-size:0.65rem; background:#333; padding:1px 4px; border-radius:3px; margin-right:3px; color:#bbb; border:1px solid #444;">${a}</span>`
        ).join('');
        card.innerHTML = `
          <div class="u-type">${u.type}</div>
          <div class="u-name">${u.name}</div>
          <div class="mt-1">${abTags}</div>
          <div class="d-flex flex-wrap mt-2">${renderStats(u)}
      ...
      UI.roster.forEach(item => {
        ...
        row.innerHTML = `
          <div class="d-flex justify-content-between">
            ...
            <button class="btn-minus" onclick="event.stopPropagation(); CCFB.ui.roster = CCFB.ui.roster.filter(x => x.id !== ${item.id}); window.CCFB.refreshUI();">−</button>
          </div>`;
    ...
    if (det) {
      ...
      if (unit) {
        ...
        if (unit.optional_upgrades?.length) {
          ...
          upTitle.className = "detail-section-title mt-3";
          upTitle.textContent = "Optional Upgrades";
          det.appendChild(upTitle);

          unit.optional_upgrades.forEach(upg => {
            const row = document.createElement("label");
            row.className = "upgrade-row d-flex align-items-center gap-2 mb-2";
            row.style.cursor = "pointer";
            const cb = document.createElement("input");
            cb.type = "checkbox";
            cb.checked = !!selectedItem.upg[upg.name];
            cb.onchange = () => {
              selectedItem.upg[upg.name] = cb.checked;
              window.CCFB.refreshUI();
            };
            row.append(cb, `${upg.name} (+${upg.cost}pts)`);
            det.appendChild(row);
          });
        }
      }
  ...
  // Ensure auto-paint if data is already there
  const tryRefresh = () => {
    if (
      window.CCFB?.ui?.fKey &&
