  ...
  function mount(rootEl, ctx) {
    ...
    window.CC_TC.openStatPanel = function(statKey) {
      ...
      panel.innerHTML = `
        ...
        <div style="padding:1.5rem;">
          ...
          <p style="color:#fff;font-size:1rem;font-weight:600;line-height:1.5;margin:0 0 1rem;">${def.desc_short}</p>
          <p style="color:#bbb;font-size:.9rem;line-height:1.7;margin:0;">${def.desc_long}</p>
        </div>`;
        ...
        <div class="cc-tc-upgrades">
          ...
          <div class="cc-tc-tag-row">
            ${(Array.isArray(unit.upgrades) ? unit.upgrades : [unit.upgrades]).map(upg => {
              ...
              const note  = typeof upg === 'object' ? (upg.desc_short || upg.desc_long || upg.note || null) : null;
    ...
    function renderSetupRound() {
      ...
      if (Array.isArray(scenarioTerrain) && scenarioTerrain.length > 0) {
        terrainTypes = scenarioTerrain.map(t => {
          ...
          return { icon: t.icon || 'fa-map-signs', label: t.name || t.label || t.type || String(t), note: t.note || t.desc_long || '', isObjective: !!(t.isObjective || t.objective) };
        });
      ...
      if (scenarioObjectives.length > 0) {
        scenarioObjectives.forEach(obj => {
          ...
          terrainTypes.push({ icon: 'fa-map-marker', label: objName, note: (obj.desc_long || obj.type || 'Scenario objective') + ' — place as marker', isObjective: true });
        });
      } else {
    ...
    function ingestAbilityFile(data) {
      ...
      Object.keys(data.abilities).forEach(slug => {
        ...
        _abilityCache[slug] = { name: slugToName(slug), id: entry._id || '', timing: entry.timing || '', short: entry.desc_short || '', long: entry.desc_long || '' };
        count++;
      });
      return count;
    }
    ...
    window.CC_TC.openAbilityPanel = function(idx) {
      ...
      var bodyHtml = entry ? `
        ...
        ${entry.desc_short ? `<p class="cc-tc-ability-short">${entry.desc_short}</p>` : ''}
        ${entry.desc_long  ? `<p class="cc-tc-ability-long">${entry.desc_long}</p>`   : ''}
