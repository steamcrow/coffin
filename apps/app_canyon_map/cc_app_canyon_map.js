...
(function () {
  ...
  // ── Core preloader bridge ────────────────────────────────────────────────
  //  Updates cc_loader_core's preloader status text + bar while it is still
  //  on screen.  This is the heaviest app so the feedback matters most here.
  //  If the core preloader has already been removed, calls are silent no-ops.
  //  text  — string to show in the pulsing label
  //  pct   — optional 0-100, overrides the CSS bar animation with a hard value
  function setCoreStatus(text, pct) {
    var s = document.getElementById('cc-preloader-status');
    if (s) s.textContent = text;
    if (typeof pct === 'number') {
      var b = document.getElementById('cc-preload-bar');
      if (b) { b.style.animation = 'none'; b.style.width = pct + '%'; }
    }
  }
  ...
  "fort-plunder": [3380, 1203, 3601, 1411],
  ...
  function renderDrawer(ui, loc) {
    ...
    if (loc.desc_long) {
      html += section("Description",
        '<p style="margin:0;line-height:1.6;color:#e8d9c4;">' + loc.desc_long + "</p>");
    }
  ...
  function mount(rootEl, userOpts) {
    ...
    function buildHitboxes() {
      ...
      locationsDoc.locations.forEach(function (loc) {
        ...
        var rect = window.L.rectangle(
          [[b[0], b[1]], [b[2], b[3]]],
          { color: "#6b3a1f", weight: 2, fillOpacity: 0.04,
            dashArray: "2, 5",
    ...
    function init() {
      ...
      var bgOverlay   = null;   // hoisted — assigned once imageOverlay is created
      var lensOverlay = null;   // hoisted — same

      showLoader();
      updateResponsiveScale();
      var loadStart = Date.now();

      setCoreStatus('Loading canyon map\u2026', 15);

      return ensureDeps(opts).then(function() {
          document.head.appendChild(knobStyleEl);

          setCoreStatus('Fetching map data\u2026', 35);
        ...
        .then(function (results) {
          ...
          bgOverlay = window.L.imageOverlay(mapDoc.map.background.image_key, bounds).addTo(bgMap);
          ...
          lensOverlay = window.L.imageOverlay(lensUrl, bounds).addTo(lensMap);
        ...
        .then(function () {
          ...
          // ── Wait for all images to actually finish downloading ────────
          // Leaflet fires a 'load' event on the imageOverlay once its <img>
          // has decoded.  We also wait for the frame PNG and the minimum
          // loader display time.  Only when all three are done do we reveal
          // the maps and dismiss the preloader — no more pop-in.

          function overlayLoaded(overlay) {
            return new Promise(function(resolve) {
              // If the underlying img is already complete, resolve immediately
              var imgEl = overlay.getElement ? overlay.getElement() : null;
              if (imgEl && imgEl.complete && imgEl.naturalWidth > 0) {
                resolve(); return;
              }
              overlay.once('load',  resolve);
              overlay.once('error', resolve); // don't stall on a broken image
            });
          }

          function frameLoaded() {
            return new Promise(function(resolve) {
              var imgEl = ui.frameEl.querySelector('img');
          ...
          setCoreStatus('Downloading map image\u2026', 80);
